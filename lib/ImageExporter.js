...
export async function exportMP4(image, result, setLoading, setError, setMessage, num_blend=500) {
  let tail_frames = num_blend * 0.3;
  const {
    createFFmpeg,
  } = FFmpeg;
  ...
  await writeCanvasToFile(canvas, '1.png', ffmpeg);

  const blendPromises = Array.from({ length: num_blend }, (_, i) => 
    blendImages(image, result, i / num_blend, false)
      .then((blendedImg) => {
        setMessage(`Writing frame ${i + 2}...`);
        writeCanvasToFile(blendedImg, `${i + 2}.png`, ffmpeg); 
      })
  );
  await Promise.all(blendPromises);
  setMessage('Rendering video...');
  const tailFramesPromises = Array.from({ length: tail_frames }, (_, i) =>
    blendImages(image, result, 1, false)
      .then((blendedImg) => writeCanvasToFile(blendedImg, `${num_blend + 2 + i}.png`, ffmpeg))
  );
  await Promise.all(tailFramesPromises);
  
  //Assign a setInterval to add '...' to the message every 500ms
