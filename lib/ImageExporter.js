...
export async function exportMP4(image, result, setLoading, setError, num_blend=50) {
  ...
  const canvas = await imgToCanvas(image, false);
  await writeCanvasToFile(canvas, '1.png');
  
  for (let i = 0; i < num_blend; i++) {
    await writeCanvasToFile(await blendImages(image, result, i / num_blend, false), `${i + 2}.png`);
  }

  for (let i = 0; i < tail_frames; i++) {
    await writeCanvasToFile(await blendImages(image, result, 1, false), `${num_blend + 2 + i}.png`);
  }
