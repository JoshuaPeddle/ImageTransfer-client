...
export async function exportVideo(image, result, setLoading, setError, num_blend=50, save=true) {
  ...
  const images = [ await imgToCanvas(image, false) ];
  for (let i = 0; i < num_blend; i++) {
    images.push(await blendImages(image, result, i / num_blend, false));
  }
  images.push( await imgToCanvas(result, false));
...
export async function exportMP4(image, result, setLoading, setError, num_blend=50) {
  ...
  const writeCanvasToFile = (canvas, filename) => {
    return new Promise((resolve) => {
  ...
  const canvas = await imgToCanvas(image, false);
  const canvas2 = await imgToCanvas(result, false);
  await writeCanvasToFile(canvas, 'tmp.image1.png');
  await writeCanvasToFile(canvas2, 'tmp.image2.png');
  // for (let i = 0; i < num_blend; i++) {
  //   const canvas3 = await blendImages(image, result, i / num_blend, false);
  //   ffmpeg.FS('writeFile', `image3-${i}.png`, new Uint8Array(canvas3.toDataURL().split(',')[1]));
  // }

  await ffmpeg.run('-framerate', '1', '-r', '1/5', '-pattern_type', 'glob', '-i', '*.png', '-c:v', 'libx264', '-pix_fmt', 'yuv420p', 'out.mp4');
